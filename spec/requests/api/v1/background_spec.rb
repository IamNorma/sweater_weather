...
describe "Backgrounds" do
  it "can fetch a background image for a city" do
    ...
    expect(json[:data][:attributes][:url]).to be_a(Hash)
    expect(json[:data][:attributes][:url]).to have_key(:raw)
    expect(json[:data][:attributes][:url][:raw]).to be_a(String)
    expect(json[:data][:attributes][:url]).to have_key(:regular)
    expect(json[:data][:attributes][:url][:regular]).to be_a(String)
  end
end
