...
describe "User" do
  it "can retrieve weather for a city" do
    ...
    expect(forecast[:data][:attributes][:hourly_weather][0]).to have_key(:predicted_temp)
    expect(forecast[:data][:attributes][:hourly_weather][0][:predicted_temp]).to be_a(Float)
